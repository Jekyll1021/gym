...
from gym.envs.robotics import inverse_peg_insert_env


# Ensure we get the path separator correct on windows
MODEL_XML_PATH = os.path.join('fetch', 'inverse_peg_insert.xml')


class CamInversePegInsertEnv(inverse_peg_insert_env.InversePegInsertEnv, utils.EzPickle):
    def __init__(self, reward_type='sparse', goal_type='fixed', cam_type='fixed', gripper_init_type='fixed', act_noise=False, obs_noise=False, depth=False, two_cam=False, use_task_index=False, random_obj=False, train_random=False, test_random=False, limit_dir=False):
        ...
        inverse_peg_insert_env.InversePegInsertEnv.__init__(
            self, MODEL_XML_PATH, block_gripper=False, n_substeps=20,
            gripper_extra_height=0.17, target_in_the_air=False, target_offset=0.0,
