...
class DrawerOpenToCloseEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        super(DrawerOpenToCloseEnv, self).__init__(
    ...
    def _get_obs(self):
        ...
        # rotations
        object_rot = rotations.mat2euler(self.sim.data.get_site_xmat('switch'))
        # velocities
        object_velp = self.sim.data.get_site_xvelp('switch') * dt
        object_velr = self.sim.data.get_site_xvelr('switch') * dt
        # gripper state
    ...
    def _reset_sim(self):
        ...
        self.sim.data.set_joint_qpos('drawer:joint', object_qpos)
    ...
    def _env_setup(self, initial_qpos):
        ...
        if self.gripper_init_type != "fixed":
            init_disturbance = np.array([self.np_random.uniform(-0.15, 0.15), self.np_random.uniform(-0.15, 0.15), 0])
        else:
    ...
    def render(self, mode='rgd_array', width=500, height=500):
        return super(DrawerOpenToCloseEnv, self).render(mode, width, height)
