...
class SlideOpenToCloseEnv(robot_env.RobotEnv):
    ...
    def _reset_sim(self):
        ...
        if self.random_obj:
            ...
            while min_color_diff < 0.1:
                ...
                if self.train_random:
                    ...
                    rgba[2] = 1
                    size = np.random.uniform(0.03, 0.035, size=3)
                else:
                    rgba *= 2
                    size = np.random.uniform(0.025, 0.035, size=3)
        ...
        if self.goal_type == "fixed":
            ...
            if self.limit_dir:
                if self.train_random:
                    offset = np.array([self.np_random.uniform(0, self.obj_range), self.np_random.uniform(0.025-self.obj_range, 0.025+self.obj_range)])
                elif self.test_random:
                    offset = np.array([self.np_random.uniform(-self.obj_range, 0), self.np_random.uniform(0.025-self.obj_range, 0.025+self.obj_range)])
            else:
    ...
    def render(self, mode='rgd_array', width=500, height=500):
        return super(SlideOpenToCloseEnv, self).render(mode, width, height)
